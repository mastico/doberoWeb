@php
    $year  = now()->year;
    $about = \App\Models\SiteSetting::get('footer_about',
        'DOBERO HOME CREATOR combines property search, relocation support, construction guidance, and local expertise to help international buyers invest with confidence on the Costa Blanca.');
@endphp

<footer class="bg-navy text-white">
    <div class="houzez-container py-16">
        <div class="grid gap-12 md:grid-cols-2">

            {{-- About Site --}}
            <div>
                {{-- Logo --}}
                <img src="{{ asset('images/logo-white.png') }}"
                     alt="Dobero Home Creator"
                     class="h-12 w-auto object-contain">
                <h3 class="mt-5 font-sans text-[18px] font-semibold text-white">{{ __('About Site') }}</h3>
                <p class="mt-4 font-body text-[14px] font-light leading-7 text-white/65 max-w-sm">
                    {{ $about }}
                </p>
                <a href="{{ locale_route('about') }}"
                   class="mt-5 inline-block font-sans text-[13px] font-medium text-primary hover:text-white transition-colors">
                    {{ __('Read More') }} →
                </a>
            </div>

            {{-- Contact Us --}}
            <div>
                <h3 class="font-sans text-[18px] font-semibold text-white">{{ __('Contact Us') }}</h3>
                <ul class="mt-5 space-y-4">
                    <li class="flex items-start gap-3 font-body text-[14px] font-light text-white/65">
                        {{-- Location --}}
                        <svg class="mt-0.5 h-5 w-5 shrink-0 text-primary" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z"/>
                        </svg>
                        <span>{{ \App\Models\SiteSetting::get('address', 'Costa Blanca, Spain') }}</span>
                    </li>
                    <li class="flex items-center gap-3 font-body text-[14px] font-light text-white/65">
                        {{-- Phone --}}
                        <svg class="h-5 w-5 shrink-0 text-primary" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 0 0 2.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 0 1-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 0 0-1.091-.852H4.5A2.25 2.25 0 0 0 2.25 4.5v2.25Z"/>
                        </svg>
                        <a href="tel:{{ \App\Models\SiteSetting::get('phone') }}"
                           class="hover:text-primary transition-colors">
                            {{ \App\Models\SiteSetting::get('phone', '+1 (800) 990 8877') }}
                        </a>
                    </li>
                    <li class="flex items-center gap-3 font-body text-[14px] font-light text-white/65">
                        {{-- Email --}}
                        <svg class="h-5 w-5 shrink-0 text-primary" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75"/>
                        </svg>
                        <a href="mailto:{{ \App\Models\SiteSetting::get('email', 'info@dobero.es') }}"
                           class="hover:text-primary transition-colors">
                            {{ \App\Models\SiteSetting::get('email', 'info@dobero.es') }}
                        </a>
                    </li>
                </ul>
            </div>
        </div>
    </div>

    {{-- Bottom bar --}}
    <div class="border-t border-white/10">
        <div class="houzez-container flex flex-col items-center justify-between gap-4 py-5 md:flex-row">
            <p class="font-body text-[13px] font-light text-white/50">
                © {{ $year }} Dobero S.L. All rights reserved.
            </p>
            {{-- Social icons --}}
            <div class="flex items-center gap-4">
                <a href="{{ \App\Models\SiteSetting::get('social_facebook', '#') }}"
                   class="text-white/50 hover:text-primary transition-colors" aria-label="Facebook">
                    <svg class="h-4 w-4 fill-current" viewBox="0 0 24 24">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                </a>
                <a href="{{ \App\Models\SiteSetting::get('social_twitter', '#') }}"
                   class="text-white/50 hover:text-primary transition-colors" aria-label="Twitter">
                    <svg class="h-4 w-4 fill-current" viewBox="0 0 24 24">
                        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                    </svg>
                </a>
                <a href="{{ \App\Models\SiteSetting::get('social_instagram', '#') }}"
                   class="text-white/50 hover:text-primary transition-colors" aria-label="Instagram">
                    <svg class="h-4 w-4 fill-current" viewBox="0 0 24 24">
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/>
                    </svg>
                </a>
                <a href="#" class="text-white/50 hover:text-primary transition-colors" aria-label="LinkedIn">
                    <svg class="h-4 w-4 fill-current" viewBox="0 0 24 24">
                        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                    </svg>
                </a>
            </div>
        </div>
    </div>
</footer>
