<div class="rounded-[2rem] bg-white p-8 shadow-sm ring-1 ring-slate-200">
    ...
    <form wire:submit="save" class="grid gap-6 lg:grid-cols-2">
        <div><label class="form-label">Title</label><input wire:key="blog-title-{{ $activeLocale }}" wire:model="form.title.{{ $activeLocale }}" class="form-input"></div>
        ...
        <div class="lg:col-span-2"><label class="form-label">Excerpt</label><textarea wire:key="blog-excerpt-{{ $activeLocale }}" wire:model="form.excerpt.{{ $activeLocale }}" rows="3" class="form-input"></textarea></div>
        <div class="lg:col-span-2"><label class="form-label">Content</label><textarea wire:key="blog-content-{{ $activeLocale }}" wire:model="form.content.{{ $activeLocale }}" rows="8" class="form-input"></textarea></div>
        ...
        <div class="lg:col-span-2 border-t border-slate-200 pt-4">
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                <div><label class="form-label">Meta Title</label><input wire:key="blog-meta-title-{{ $activeLocale }}" wire:model="form.meta_title.{{ $activeLocale }}" class="form-input" placeholder="Override page title for search engines"></div>
                <div><label class="form-label">Meta Description</label><textarea wire:key="blog-meta-description-{{ $activeLocale }}" wire:model="form.meta_description.{{ $activeLocale }}" rows="2" class="form-input" placeholder="160-character summary for search results"></textarea></div>
            </div>
        </div>
